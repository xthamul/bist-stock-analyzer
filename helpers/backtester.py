    ...
    def next(self):
        if crossover(self.ema1, self.ema2):
            if not self.position:
                sl = (
                    self.data.Close[-1] * (1 - self.stop_loss)
                    if self.stop_loss
                    else None
                )
                tp = (
                    self.data.Close[-1] * (1 + self.take_profit)
                    if self.take_profit
                    else None
                )
...
class BBandStrategy(Strategy):
    ...
    def init(self):
        bbands = self.I(
            ta.bbands, pd.Series(self.data.Close), length=self.length, std=self.std
        )
...
def run_backtest(
    strategy_class, data, cash=100000, commission=0.002, **strategy_params
):
