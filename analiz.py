...
class StockAnalyzerApp:
    ...
    def run_analysis(self, event=None):
        ...
        try:
            ...
            veri_filtrelenmis = filter_data_by_date(veri_hesaplanmis, time_delta=time_delta)

            if veri_filtrelenmis.empty:
                self.log_message("Hata: Seçilen görüntüleme aralığı için yeterli veri yok.")
                return

            self.log_message("Grafik oluşturuluyor...")
            self.display_chart(veri_filtrelenmis, hisse_kodu_yf, interval_display, analysis_type)
            
            self.log_message("Teknik analiz özeti oluşturuluyor...")
            technical_summary = generate_technical_summary(veri_filtrelenmis, as_markdown=False)
            self.log_message("\n--- Teknik Analiz Özeti ---\n" + technical_summary)
    ...
    def display_fundamental_data(self, hisse_kodu_yf):
        try:
            ...
            summary_text, key_info_text = generate_fundamental_summary(info, as_markdown=False)
            
            display_text = f"--- {info.get('longName', hisse_kodu_yf)} Temel Verileri ---\n\n"
